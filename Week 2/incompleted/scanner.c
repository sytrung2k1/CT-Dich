...
Token *getToken(void)
{
  ...
  case CHAR_PLUS:
    ...
    return token;
    // ....
  case CHAR_LPAR:                              // có 3 TH là (*, (. và (
    token = makeToken(SB_LPAR, lineNo, colNo); // ban đầu sẽ là (
    readChar();
    if (charCodes[currentChar] == CHAR_TIMES) // nếu kí tự sau ( là *
    {
      skipComment();
      return getToken();
    }
  ...
  case CHAR_LT: // dấu <
    ...
    readChar();
    if (charCodes[currentChar] == CHAR_EQ) // nếu kí tự sau là '=' thì sẽ là <=(SB_LE) (lesness or equalyti : bé hơn hoặc bằng)
    {
  ...
  case CHAR_GT: // Dấu >
    ...
    { // Nếu kí tự sau là '=' thì sẽ là >= (SB_GE) (greaterness or equality : lớn hơn hoặc bằng)
  ...
  case CHAR_EXCLAIMATION: // Dấu !
    ...
    readChar();
    if (charCodes[currentChar] == CHAR_EQ)
    { // Nếu kí tự tiếp theo là '=' thì sẽ là != (SB_NEQ)
  ...
  case CHAR_COLON: // Dấu :
    ...
    readChar();
    if (charCodes[currentChar] == CHAR_EQ)
    { // Nếu kí tự tiếp theo là '=' thì sẽ là := (SB_ASSIGN)
